...
class ResNet(nn.Module):
    ...
    def forward(self, x):
        ...
        E_cen = self.attention(x_flat)
        A_cen = self.attention_heads(E_cen).reshape(-1, 121, 2).softmax(dim=-1)[:, :, 1] #512

        E_tripPos = self.attention(x_flat)
        A_tripPos = self.attention_heads(E_tripPos).reshape(-1, 121, 2).softmax(dim=-1)[:, :, 1] #512

        E_tripNeg = self.attention(x_flat)
        A_tripNeg = self.attention_heads(E_tripNeg).reshape(-1, 121, 2).softmax(dim=-1)[:, :, 1] #512

        x = self.arrangement(x)
        x, alpha = self.arm(x)
        f_cen = torch.flatten(x, 1)
        f_trip = torch.flatten(x, 1)
        out = self.fc(f_cen)
    ...
    if not pretrained == '':
        ...
        if pretrained == 'imagenet':
            #state_dict = load_state_dict_from_url(model_urls[arch],
                                                  #progress=progress)                                                  
            imagenet_model = torch.load("resnet18-5c106cde.pth")
            pretrained_state_dict = imagenet_model
            ...
            pretrained_state_dict = msceleb_model
            state_dict = model.state_dict()
            loaded_keys = 0
            total_keys = 0
            for key in pretrained_state_dict:
            ...
            msceleb_model = torch.load('epoch27_acc0.9078.pth')
            pretrained_state_dict = msceleb_model
            state_dict = model.state_dict()
            loaded_keys = 0
            total_keys = 0
